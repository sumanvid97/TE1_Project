...
svmGrid <- expand.grid(C = seq(0.01,0.1, 0.01)) 
# svmGrid <- expand.grid(C = 0.01) 

svmmodel <- train( Accept ~ Speed..km.hr. + Spatial.Gap, data = train, method = "svmLinear", 
                   trControl=trctrl, preProcess = c("center", "scale"), 
                   tuneGrid = svmGrid) # training the model

prediction <- predict(svmmodel, test) # predicting the gap acceptance from test dataset
...
#the following code block was used to determine the most efficient train-test split configuration storing it as the seed value
...
#  test <- data[-index,]
#  trctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 3)
#  grid <- expand.grid(C = seq(0.01,0.1, 0.01))
#  model <- train( Accept ~ Speed..km.hr. + Spatial.Gap, data = train, method = "svmLinear",
#                  trControl=trctrl,
#                  preProcess = c("center", "scale"), tuneGrid = grid,
#                  tuneLength = 10)
#  prediction <- predict(model, test)
#  confusionMatrix(prediction, test$Accept)
#  accuracy = mean(prediction == test$Accept)
#  if(accuracy > max){
#    max = accuracy
#    bestseed = 1200+i
#  }
